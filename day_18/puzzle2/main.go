...
// Ensure that the four corners of the grid are always on.
func keepCornersOn(grid [][]rune) {
	grid[0][0] = '#'
	grid[0][gridSize-1] = '#'
	grid[gridSize-1][0] = '#'
...
func simulateStep(grid [][]rune) [][]rune {
	...
	// Keep the four corners always on
	keepCornersOn(newGrid)
	return newGrid
}
...
func main() {
	...
	// Ensure corners are on in the initial grid
	keepCornersOn(grid)
